...
fn resolve_temp_dir(settings: &Settings) -> PathBuf {
    let temp_dir = settings.main.get("temp_dir")
        .map(|s| normalize_path(s.trim()))
        .unwrap_or_default();

    if temp_dir.is_empty() {
        env::temp_dir().join("wb-toolsloader")
    } else {
        PathBuf::from(&temp_dir).join("wb-toolsloader")
    }
}

fn cleanup_temp_dir(dir: &Path) -> Result<(), Box<dyn std::error::Error>> {
    if dir.exists() {
        fs::remove_dir_all(dir)?;
    }
    Ok(())
